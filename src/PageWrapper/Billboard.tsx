...
export const Billboard = React.forwardRef((props, ref) => {
    ...
    const [recordsRef, setRecords] = useSyncState<IBillboardState>(
        new Map([
            [RecordRegion.Chat, []],
    ...
    const clearPlayerLogs = () => {
        if (recordsRef.current) {
            setRecords(
                recordsRef.current
                    .set(RecordRegion.HarePlayer, [])
                    .set(RecordRegion.TortoisePlayer, [])
            );
        }
    };
    ...
    const clearRaceTrackLogs = () => {
        if (recordsRef.current) {
            setRecords(
                recordsRef.current
                    .set(RecordRegion.HareRaceTrack, [])
                    .set(RecordRegion.TortoiseRaceTrack, [])
            );
        }
    };
    ...
    return (
        <div className="p-2" style={{ border: "solid", height: "80vh" }}>
            <div style={{ height: "30%", overflow: "auto" }}>
                ...
                >
                    <div style={{ flex: 1 }}>
                        <RecordList records={getRecords(RecordRegion.Chat)} />
                    </div>
                </div>
            </div>
            <div style={{ height: "35%", borderTop: "solid" }}>
                ...
                >
                    <div style={{ flex: 1 }}>
                        <RecordList
                            records={getRecords(RecordRegion.HarePlayer)}
                        />
                    </div>
                    <div style={{ flex: 1 }}>
                        <RecordList
                            records={getRecords(RecordRegion.TortoisePlayer)}
                        />
                    </div>
                </div>
            </div>
            <div style={{ height: "35%", borderTop: "solid" }}>
                ...
                >
                    <div style={{ flex: 1 }}>
                        <RecordList
                            records={getRecords(RecordRegion.HareRaceTrack)}
                        />
                    </div>
                    <div style={{ flex: 1 }}>
                        <RecordList
                            records={getRecords(RecordRegion.TortoiseRaceTrack)}
                        />
                    </div>
                </div>
            </div>
